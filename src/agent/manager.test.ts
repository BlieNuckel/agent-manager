...
describe('AgentSDKManager', () => {
  ...
  describe('worktree signal detection', () => {
    it('emits worktreeCreated event when signal detected in text', async () => {
      vi.mocked(query).mockImplementation(() =>
        createMockQuery([
          {
            type: 'assistant',
            parent_tool_use_id: null,
            message: {
              content: [{ type: 'text', text: '[WORKTREE_CREATED] feature-branch' }],
            },
          },
        ]) as any
      );

      const worktreeCreatedHandler = vi.fn();
      manager.on('worktreeCreated', worktreeCreatedHandler);
      manager.on('idle', vi.fn());

      await manager.spawn('test-id', 'test prompt', '/test/dir', 'normal');

      await vi.waitFor(() => {
        expect(worktreeCreatedHandler).toHaveBeenCalledWith('test-id', 'feature-branch');
      });
    });

    it('emits mergeReady event when signal detected', async () => {
      vi.mocked(query).mockImplementation(() =>
        createMockQuery([
          {
            type: 'assistant',
            parent_tool_use_id: null,
            message: {
              content: [{ type: 'text', text: '[WORKTREE_MERGE_READY] feature-branch' }],
            },
          },
        ]) as any
      );

      const mergeReadyHandler = vi.fn();
      manager.on('mergeReady', mergeReadyHandler);
      manager.on('idle', vi.fn());

      await manager.spawn('test-id', 'test prompt', '/test/dir', 'normal');

      await vi.waitFor(() => {
        expect(mergeReadyHandler).toHaveBeenCalledWith('test-id', 'feature-branch');
      });
    });

    it('emits mergeConflicts event when signal detected', async () => {
      vi.mocked(query).mockImplementation(() =>
        createMockQuery([
          {
            type: 'assistant',
            parent_tool_use_id: null,
            message: {
              content: [{ type: 'text', text: '[WORKTREE_MERGE_CONFLICTS] feature-branch' }],
            },
          },
        ]) as any
      );

      const mergeConflictsHandler = vi.fn();
      manager.on('mergeConflicts', mergeConflictsHandler);
      manager.on('idle', vi.fn());

      await manager.spawn('test-id', 'test prompt', '/test/dir', 'normal');

      await vi.waitFor(() => {
        expect(mergeConflictsHandler).toHaveBeenCalledWith('test-id', 'feature-branch');
      });
    });

    it('emits mergeFailed event when signal detected with error', async () => {
      vi.mocked(query).mockImplementation(() =>
        createMockQuery([
          {
            type: 'assistant',
            parent_tool_use_id: null,
            message: {
              content: [{ type: 'text', text: '[WORKTREE_MERGE_FAILED] feature-branch Git error occurred' }],
            },
          },
        ]) as any
      );

      const mergeFailedHandler = vi.fn();
      manager.on('mergeFailed', mergeFailedHandler);
      manager.on('idle', vi.fn());

      await manager.spawn('test-id', 'test prompt', '/test/dir', 'normal');

      await vi.waitFor(() => {
        expect(mergeFailedHandler).toHaveBeenCalledWith('test-id', 'feature-branch', 'Git error occurred');
      });
    });

    it('emits mergeCompleted event when signal detected', async () => {
      vi.mocked(query).mockImplementation(() =>
        createMockQuery([
          {
            type: 'assistant',
            parent_tool_use_id: null,
            message: {
              content: [{ type: 'text', text: '[WORKTREE_MERGED] feature-branch' }],
            },
          },
        ]) as any
      );

      const mergeCompletedHandler = vi.fn();
      manager.on('mergeCompleted', mergeCompletedHandler);
      manager.on('idle', vi.fn());

      await manager.spawn('test-id', 'test prompt', '/test/dir', 'normal');

      await vi.waitFor(() => {
        expect(mergeCompletedHandler).toHaveBeenCalledWith('test-id', 'feature-branch');
      });
    });

    it('detects worktree signals in tool result output', async () => {
      vi.mocked(query).mockImplementation(() =>
        createMockQuery([
          {
            type: 'user',
            tool_use_result: '[WORKTREE_CREATED] my-feature',
          },
        ]) as any
      );
      ...
      await vi.waitFor(() => {
        expect(worktreeCreatedHandler).toHaveBeenCalledWith('test-id', 'my-feature');
      });
    });
  });
